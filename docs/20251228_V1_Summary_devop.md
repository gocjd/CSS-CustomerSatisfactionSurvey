...
| 날짜 | 버전 | 변경 내용 | 작성 주체 |
|------|------|----------|----------|
| 2025-12-28 | v1.4 | 미니맵 도킹 시스템 구현, 초기 위치 최적화(좌하단), 시인성 개선 | AI Assistant |
...
#### A. 시각적 편의성 및 배치 최적화
- **초기 위치 조정**: 미니맵 실행 시 초기 위치를 **왼쪽 하단**으로 변경하여 조작 팔레트와의 접근성 강화.
- **z-index 상향 (999)**: 플로팅 미니맵이 사이드 패널이나 팔레트 뒤로 숨지 않도록 레이어링 순위 최적화.
- **헤더 UI 확장**: 헤더 높이를 키우고, 설문 제목을 더 크고 굵게 표시하여 식별력 강화.

#### B. 도킹(Docking) 및 분리 시스템
- **자동 도킹 로직**: 미니맵을 왼쪽 팔레트 영역으로 드래그 시 하단 섹션에 자석처럼 고정되는 기능 구현.
- **포털(Portal) 기반 렌더링**: 도킹 상태에 따라 DOM 구조를 동적으로 변경하여 팔레트 구성 요소와 완벽히 결합.
- **실시간 반응형 감지**: 도킹 대상 엘리먼트를 상시 배치하여 드래그 중 도킹 가능 영역을 즉각적으로 시각화.
...
- [x] **MiniMap Docking**: 왼쪽 팔레트 하단 자동 도킹 및 분리 시스템.
- [x] **Enhanced MiniMap**: 좌하단 배치, z-index 999 상향, 헤더 확장.
